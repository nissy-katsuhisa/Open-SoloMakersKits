...
- [skills/app-product-summary/SKILL.md](../app-product-summary/SKILL.md) を使って、プロダクト理解とブランド理解を実データから抽出する
...
- `output/app-store-screenshots/final-assets/` 配下に App Store / Play Store 用の最終画像を生成する
...
- 入力: `output/app-store-screenshots/screenshot-plan.json` または `app-store-screenshots.json`
- 検査内容:
...
- `../app-product-summary/SKILL.md`
  - コードベースを読み、プロダクト情報とブランド情報を統合した `app info JSON` を出力する
...
- まず `../app-product-summary/SKILL.md` でアプリコードを読み、デッキ設計前の前提を固める
- この skill 自体が Build iOS Apps workflow を使って simulator を起動し、スクリーンショットを撮る
- Mac にインストール済みの simulator runtime をそのまま使う
- `output/product-analysis/`, `output/ios-page-screenshots/`, `output/app-store-screenshots/` のような出力フォルダへ整理する
...
- `output/product-analysis/<app-slug>-app-info.json`
- `output/ios-page-screenshots/<screen-slug>.png`
- `output/app-store-screenshots/screenshot-plan.md`
- `output/app-store-screenshots/screenshot-plan.json`
- `output/app-store-screenshots/final-assets/apple/iphone/6.9/*.png`
- `output/app-store-screenshots/final-assets/apple/ipad/13/*.png`
- `output/app-store-screenshots/final-assets/android/phone/*.png`
- `output/app-store-screenshots/final-assets/android/feature-graphic/*.png`
