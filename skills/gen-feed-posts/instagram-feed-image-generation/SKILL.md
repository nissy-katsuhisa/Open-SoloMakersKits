...
これは `skills/app-product-summary/` が生成する、後段 skill 共通の app product context として扱う。
...
script が OK を返した場合は、既存の `data/app-product-context.json` をそのまま使用する。  
script が missing を返した場合は、先に `skills/app-product-summary/` を使用して `data/app-product-context.json` を生成し、もう一度 script で確認してから続行する。
...
- script が OK なら、既存の `data/app-product-context.json` を読む
- script が missing を返した場合は、先に `skills/app-product-summary/` を使って作成し、再確認してから読む
- 何のアプリか
- 主要機能は何か
- どんな価値があるか
- どんなコピー候補があるか
