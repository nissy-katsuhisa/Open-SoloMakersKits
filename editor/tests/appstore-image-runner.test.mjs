...
test("複数生成結果を横スクロール表示向けの順序付き画像リストとして返す", () => {
  ...
  assert.ok(images.every((image) => image.sourceSvgPath.startsWith("output/app-store-screenshot-work/run-1/svg/")));
});
...
test("AppStore画像runnerがoutput配下へplanと最終画像を書き出す", async () => {
  ...
  try {
    ...
    const sourceSvg = await readFile(path.join(workDir, result.run.id, "svg", "01.svg"), "utf8");
